...
.PARAMETER WorkPath
    Arbeitsverzeichnis fuer den unverschluesselten Zwischenstand. Standard ist
    das Temp-Verzeichnis des ausfuehrenden Kontos.
...
$outDump   = Join-Path $WorkPath "$Database-$stamp.dump"
...
try
{
    ...
    # Dateioperationen auf dem Zwischenstand laufen ueber .NET: Windows
    # PowerShell 5.1 scheitert mit Remove-Item -LiteralPath an 8.3-Kurzpfaden
    # (z.B. C:\Users\P0198~1.FAL\...), die als TEMP durchaus vorkommen.
    if (-not [System.IO.File]::Exists($outDump))
    {
        throw "Dump '$outDump' wurde nicht erzeugt."
    }

    $rohGroesse = (New-Object System.IO.FileInfo $outDump).Length
    ...
    $mbRoh  = '{0:N1} MB' -f ($rohGroesse / 1mb)
    $mbZiel = '{0:N1} MB' -f ($ziel.Length / 1mb)

    Write-Log "OK $stamp Datei=$($ziel.Name) Groesse=$mbZiel ($($ziel.Length) Byte, Dump $mbRoh) SHA256=$hash Dauer=${dauer}s"
...
finally
{
    ...
    if ([System.IO.File]::Exists($outDump))
    {
        try
        {
            [System.IO.File]::Delete($outDump)
        ...
        {
            Write-Log "Unverschluesselter Zwischenstand '$outDump' konnte NICHT entfernt werden: $($_.Exception.Message)" -Stufe FEHLER
        }
    }
}
