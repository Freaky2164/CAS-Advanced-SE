...
public class CDisplayField extends JPanel implements CInfoParent {
	...
	private CDisplayFieldBean display;
	private CInfoFrame frame;
	...
	public CDisplayField(CProperties p, CInfoFrame frame) {
		...
		this.name = p.get("owner") + "." +
				p.get("table_name") + "." +
				p.get("column_name");
		if (p.get("data_type").toString().equals("TEXT")) {
			display = new CDisplayFieldTextBean(p, frame);
		} else if (p.get("data_type").toString().equals("TABLE")) {
			display = new CDisplayFieldTableBean(p, frame);
		} else if (p.get("data_type").toString().equals("DATE")) {
			display = new CDisplayFieldDateBean(p, frame);
		} else if (p.get("data_type").toString().equals("FLOAT")) {
			display = new CDisplayFieldNumberBean(p, frame);
		} else if (p.get("data_type").toString().equals("CHECK")) {
			display = new CDisplayFieldCheckBean(p, frame);
		} else if (p.get("data_type").toString().equals("DOCUMENT")) {
			display = new CDisplayFieldDocumentBean(p, frame);
		} else if (p.get("data_type").toString().equals("HISTORY")) {
			display = new CDisplayFieldHistoryBean(p, frame);
		} else if (p.get("data_type").toString().equals("LIST")) {
			display = new CDisplayFieldListBean(p, frame);
		} else if (p.get("data_type").toString().equals("LINK")) {
			display = new CDisplayFieldLinkBean(p, frame);
		} else if (p.get("data_type").toString().equals("CHARFRAME")) {
...
/*		if (display.getTextField() != null) {
            display.getTextField().addFocusListener(new FocusListener() {
