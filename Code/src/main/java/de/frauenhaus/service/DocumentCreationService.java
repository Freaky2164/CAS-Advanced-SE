...
public class DocumentCreationService {
     ...
     * @throws ResponseStatusException mit Status 404, wenn das Bußgeld nicht existiert
     */
    public byte[] bussgeldBestaetigung(Long bussgeldId) {
        Bussgeld b = bussgelder.findById(bussgeldId)
                .orElseThrow(() -> new ResponseStatusException(NOT_FOUND, "Bußgeld " + bussgeldId + " nicht gefunden"));
     ...
     * @throws ResponseStatusException mit Status 404, wenn die Spende nicht existiert
     */
    public byte[] spendenBescheinigung(Long spendeId) {
        Spende spende = spenden.findById(spendeId)
                .orElseThrow(() -> new ResponseStatusException(NOT_FOUND, "Spende " + spendeId + " nicht gefunden"));
     ...
     * Die Prüfung ignoriert Groß-/Kleinschreibung, damit sie sich wie die
     * Trägerauswertung im {@link VerteilerService} verhält.
     */
    private static String praefix(String verein) {
        return "Förderverein".equalsIgnoreCase(verein) ? "FV" : "FH";
    }
