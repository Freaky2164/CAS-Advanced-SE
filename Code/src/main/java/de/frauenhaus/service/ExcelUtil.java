...
final class ExcelUtil {
     ...
     * Ein Blatt ohne Kopfzeile wird ohne Breitenanpassung geschrieben, statt
     * mit einer NullPointerException abzubrechen.
     *
    ...
    static byte[] toBytes(Workbook wb) {
        try (wb; ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            Row kopfzeile = wb.getSheetAt(0).getRow(0);
            if (kopfzeile != null) {
                for (int i = 0; i < kopfzeile.getLastCellNum(); i++) {
