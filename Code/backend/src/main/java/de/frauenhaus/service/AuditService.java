...
/**
 * @author Nils
 *     <p>Liefert die Änderungshistorie auditierter Stammdaten-Datensätze aus Hibernate Envers als
 *     einfache REST-taugliche DTOs mit Feld-Diffs.
 */
@Service
  ...
  public List<VerlaufEintrag> verlauf(Class<?> entityClass, Object id) {
    ...
    if (revisionen.isEmpty() && entityManager.find(entityClass, id) == null) {
      throw new ResponseStatusException(
          NOT_FOUND, entityClass.getSimpleName() + " " + id + " nicht gefunden");
    }
    ...
    for (Object[] revision : revisionen) {
      ...
      String benutzername =
          revisionInfo.getUsername() != null ? revisionInfo.getUsername() : "system";
      eintraege.add(
          new VerlaufEintrag(
              revisionInfo.getRev(),
    ...
    ReflectionUtils.doWithFields(
        ...
        feld -> {
          if (Modifier.isStatic(feld.getModifiers())
              || Modifier.isTransient(feld.getModifiers())
              || feld.isSynthetic()
              || feld.isAnnotationPresent(jakarta.persistence.Id.class)
