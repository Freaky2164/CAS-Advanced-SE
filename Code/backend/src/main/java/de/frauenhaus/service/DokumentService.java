...
/**
 * @author Nils
 *     <p>Verwaltung der Dokument-Anhänge zu Stammdaten inkl. Upload/Download und Validierung der
 *     erlaubten Ziel-Entitäten.
 */
@Service
@Transactional
public class DokumentService {
  ...
  public record DokumentMetadaten(
      Long id,
      Dokument.EntityTyp entityTyp,
      String entityId,
      String dateiname,
      String contentType,
      long groesse,
      OffsetDateTime hochgeladenAm,
      String hochgeladenVon) {

    static DokumentMetadaten of(Dokument d) {
      return new DokumentMetadaten(
          d.getId(),
          d.getEntityTyp(),
          d.getEntityId(),
          d.getDateiname(),
  ...
  public DokumentMetadaten hochladen(String entityTyp, String entityId, MultipartFile datei) {
    ...
    try {
      ...
      throw new ResponseStatusException(
          INTERNAL_SERVER_ERROR, "Datei konnte nicht gelesen werden", e);
    }
  ...
  public List<DokumentMetadaten> liste(String entityTyp, String entityId) {
    ...
    String normalisierteEntityId = pruefeEntityId(typ, entityId);
    return dokumente
        .findMetadatenByEntityTypAndEntityIdOrderByHochgeladenAmDesc(typ, normalisierteEntityId)
        .stream()
  ...
  public DokumentDownload herunterladen(Long id) {
    ...
    return new DokumentDownload(
        dokument.getDateiname(), dokument.getContentType(), dokument.getInhalt());
  }
  ...
  private Dokument holen(Long id) {
    return dokumente
        .findById(id)
        .orElseThrow(
            () -> new ResponseStatusException(NOT_FOUND, "Dokument " + id + " nicht gefunden"));
  }
  ...
  private Dokument.EntityTyp pruefeEntityTyp(String entityTyp) {
    ...
    try {
      ...
      throw new ResponseStatusException(
          BAD_REQUEST,
          "Ungültiger entityTyp '"
              + entityTyp
              + "'. Erlaubt: MITGLIED, VEREIN, BUSSGELD, SPENDE, GERICHT");
    }
  }
  ...
  private static String dateiname(MultipartFile datei) {
    String dateiname =
        StringUtils.cleanPath(
            datei.getOriginalFilename() != null ? datei.getOriginalFilename() : "");
  ...
  private static String contentType(MultipartFile datei) {
    return StringUtils.hasText(datei.getContentType())
