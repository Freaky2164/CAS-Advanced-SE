...
/**
 * @author Nils
 *     <p>Pflege der Bußgelder inkl. Zahlungseingänge (alt: generisches CInfoFrame auf
...
public class BussgeldService {
  ...
  /**
   * @author Nils
   *     <p>Zahlungseingang ohne Rückverweis auf das Bußgeld.
   */
  ...
  /**
   * @author Nils
   *     <p>Bußgeld mit aufgelösten Zahlungseingängen statt lazy Collection.
   */
  public record BussgeldResponse(
      Long id,
      Long gerichtId,
      String gerichtBezeichnung,
      String verein,
      String status,
      String name,
      String vorname,
      ...
      boolean bezahlt,
      String bemerkung,
      List<EingangResponse> eingaenge) {
  ...
  public BussgeldService(
      BussgeldRepository bussgelder,
      GerichtRepository gerichte,
      VereinRepository vereine,
      BussgeldstatusRepository stati,
  ...
  public BussgeldResponse anlegen(
      Long gerichtId,
      String verein,
      String status,
      String name,
  ...
  public BussgeldResponse aendern(
      Long id,
      Long gerichtId,
      String verein,
      String status,
  ...
  /**
   * @author Nils
   *     <p>Fügt einen Zahlungseingang hinzu (alt: CInfoFrameStatusSub -> neue Zeile in
   *     frauenhaus.eingang).
   */
  public BussgeldResponse eingangHinzufuegen(
      Long bussgeldId, LocalDate datum, BigDecimal betrag, String bemerkung) {
  ...
  /**
   * @author Nils
   *     <p>Entfernt einen Zahlungseingang (orphanRemoval löscht ihn beim Speichern).
   */
  public BussgeldResponse eingangEntfernen(Long bussgeldId, Long eingangId) {
    ...
    if (!entfernt) {
      throw new ResponseStatusException(
          NOT_FOUND, "Zahlungseingang " + eingangId + " nicht gefunden");
    }
    return BussgeldResponse.of(b);
  }
  ...
  private Bussgeld holen(Long id) {
    return bussgelder
        .findById(id)
        .orElseThrow(
            () -> new ResponseStatusException(NOT_FOUND, "Bußgeld " + id + " nicht gefunden"));
  }
  ...
  private void uebertragen(
    ...
    Gericht gericht =
        gerichte
            .findById(gerichtId)
            .orElseThrow(
                () -> new ResponseStatusException(BAD_REQUEST, "Unbekanntes Gericht " + gerichtId));
    Verein v =
        vereine
            .findById(verein)
            .orElseThrow(
