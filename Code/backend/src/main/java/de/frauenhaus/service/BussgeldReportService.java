...
/**
 * @author Nils
 *     <p>Bußgeld-Reports, portiert aus CReportBussgeldUebersicht, CReportBussgeldDetail und
 *     CCommandBestaetigungBussgeld (Word-COM → docx via POI).
 */
@Service
...
public class BussgeldReportService {
  ...
  /**
   * @author Nils
   *     <p>Übersicht: je Träger die Summen pro Gericht (Bußgelder vs. Zahlungseingänge).
   */
  public byte[] uebersicht(LocalDate von, LocalDate bis) {
    ...
    for (String verein : VEREINE) {
      ...
      for (BussgeldRepository.UebersichtZeile z : bussgelder.uebersicht(von, bis, verein)) {
        ExcelUtil.zeile(
            sheet, line++, List.of(z.getBezeichnung(), z.getBussgelder(), z.getEingaenge()));
  ...
  /**
   * @author Nils
   *     <p>Detail: Bußgelder mit allen Zahlungseingängen im Zeitraum.
   */
  ...
  public byte[] bestaetigung(Long bussgeldId) {
    ...
    wordTemplate.absatz(
        ...
        "Bußgeld: "
            ...
            + b.getVorname()
            + " über "
            + b.getBetrag().setScale(2, RoundingMode.HALF_UP)
            + " EUR vom "
            + DATUM.format(b.getDatum()));
    wordTemplate.leerzeile(doc);
    ...
    for (Eingang e : b.getEingaenge()) {
      wordTemplate.absatz(
          ...
          DATUM.format(e.getDatum())
              + "  "
              + e.getBetrag().setScale(2, RoundingMode.HALF_UP)
              + " EUR");
    }
    wordTemplate.leerzeile(doc);
