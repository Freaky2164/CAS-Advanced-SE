...
/**
 * @author Nils
 *     <p>Erzeugt Dokumente aus den Word-Vorlagen in {@code vorlagen/} (alt:
 *     CCommandBestaetigungBussgeld und CCommandSpendenQuittung mit Word-COM): Bußgeldbestätigungen
 *     aus FHBG.dot bzw. FVBG.dot, Spendenbescheinigungen aus den FHSB- bzw. FVSB-Vorlagen je Träger
 *     und Spendentyp. Befüllt werden die Lesezeichen der Vorlagen, das Ergebnis ist ein
 *     Word-Dokument (.doc).
 */
@Service
...
public class DocumentCreationService {
  ...
  /**
   * @author Nils
   *     <p>Vorlagen-Namenszusatz je Spendentyp (FHSB + Zusatz + ".dot").
   */
  private static final Map<String, String> VORLAGE_JE_SPENDENTYP =
      Map.of(
  ...
  /**
   * @author Nils
   *     <p>Zahlungsbestätigung an das Gericht (Vorlage FHBG.dot bzw. FVBG.dot): Anschrift des
   *     Gerichts, Strafsache, Bußgeldbetrag sowie die Liste der Zahlungseingänge mit
   *     Restsummen-Hinweis.
   */
  public byte[] bussgeldBestaetigung(Long bussgeldId) {
    Bussgeld b =
        bussgelder
            .findById(bussgeldId)
            .orElseThrow(
                () -> new IllegalArgumentException("Bußgeld " + bussgeldId + " nicht gefunden"));
    NumberFormat waehrung = NumberFormat.getCurrencyInstance(Locale.GERMANY);

    List<Eingang> eingaenge =
        b.getEingaenge().stream().sorted(Comparator.comparing(Eingang::getDatum)).toList();
    BigDecimal gezahlt =
        eingaenge.stream().map(Eingang::getBetrag).reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    return DocumentCreationHelpers.fuelleVorlage(
        vorlage(praefix(b.getVerein().getName()) + "BG.dot"), werte);
  }

  /**
   * @author Nils
   *     <p>Spendenbescheinigung (Vorlagen FHSB*.dot bzw. FVSB*.dot je Träger und Spendentyp). Bei
   *     Dauerspenden werden alle Einzelspenden des Jahres summiert und als Liste am Lesezeichen
   *     {@code einzelbetrag} eingesetzt (alt: fillDonationSummary), inklusive Betrag in Worten.
   */
  public byte[] spendenBescheinigung(Long spendeId) {
    Spende spende =
        spenden
            .findById(spendeId)
            .orElseThrow(
                () -> new IllegalArgumentException("Spende " + spendeId + " nicht gefunden"));
    ...
    List<Spende> einzelspenden =
        "Geldspende dauer".equalsIgnoreCase(spendentyp)
            ? spenden.findJahresspenden(m.getId(), spende.getDatum().getYear(), spendentyp, verein)
            : List.of(spende);
    BigDecimal summe =
        einzelspenden.stream().map(Spende::getBetrag).reduce(BigDecimal.ZERO, BigDecimal::add);
  ...
  /**
   * @author Nils
   *     <p>FHSB<Typ>.dot bzw. FVSB<Typ>.dot; fehlt die typspezifische Vorlage, die allgemeine
   *     FHSB.dot/FVSB.dot.
   */
  ...
  /**
   * @author Nils
   *     <p>Vorlagen-Präfix des Trägers: Förderverein -> FV, sonst FH.
   */
  ...
  /**
   * @author Nils
   *     <p>Absoluter Pfad einer Vorlage; schlägt mit klarer Meldung fehl, wenn sie fehlt.
   */
  private Path vorlage(String dateiname) {
    Path pfad = vorlagen.resolve(dateiname);
    if (!Files.exists(pfad)) {
      throw new IllegalStateException(
          "Vorlage " + pfad + " nicht gefunden – app.vorlagen.pfad prüfen");
    }
    return pfad;
  }
  ...
  /**
   * @author Nils
   *     <p>Hinweistext zum noch offenen Betrag (alt: Restsummen-Logik der Bußgeldbestätigung).
   */
  private static String restsummenText(
      BigDecimal betrag, BigDecimal gezahlt, NumberFormat waehrung) {
    if (gezahlt.signum() == 0) {
