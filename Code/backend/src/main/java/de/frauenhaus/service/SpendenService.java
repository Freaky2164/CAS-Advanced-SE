...
/**
 * @author Nils
 *     <p>Spendenübersicht und Spendenquittungen, portiert aus CReportSpendenUebersicht,
 *     CReportSpendenQuittungen und CCommandSpendenQuittung.
 */
@Service
...
public class SpendenService {
  ...
  public SpendenService(
      SpendeRepository spenden, MitgliedRepository mitglieder, WordTemplateService wordTemplate) {
  ...
  /**
   * @author Nils
   *     <p>Alle Spenden eines Jahres, gruppiert nach Träger/Spendentyp/-art.
   */
  ...
  /**
   * Quittungsdaten zu einer Spende. Bei Dauerspenden werden alle Einzelspenden des Jahres summiert
   * (alt: fillDonationSummary), inkl. Betrag in Worten.
   */
  public byte[] quittung(Long spendeId) {
    ...
    for (Mitglied m : mitglieder.findBySpende(spendeId)) {
      List<Spende> einzelspenden =
          "Dauerspende".equalsIgnoreCase(spendentyp)
              ? spenden.findJahresspenden(
                  m.getId(), spende.getDatum().getYear(), spendentyp, verein)
              : List.of(spende);
      ...
      ExcelUtil.zeile(
          ...
          java.util.Arrays.asList(
              ...
              verein,
              spendentyp,
              DATUM.format(spende.getDatum()),
              summe,
              BetragInWorten.von(summe),
              String.join("\n", betraege),
              String.join("\n", bemerkungen)));
    }
    return ExcelUtil.toBytes(wb);
  ...
  public byte[] quittungDocx(Long spendeId) {
    ...
    for (int i = 0; i < empfaenger.size(); i++) {
      ...
      wordTemplate.adresse(
          doc,
          (m.getAnrede() == null ? "" : m.getAnrede() + " ") + m.getVorname() + " " + m.getName(),
          m.getStrasse(),
          (m.getPlz() == null ? "" : m.getPlz() + " ") + m.getOrt());
      wordTemplate.leerzeile(doc);
      ...
      for (Spende s : einzelspenden) {
        wordTemplate.absatz(
            doc, "  " + DATUM.format(s.getDatum()) + "  " + s.getBetrag().setScale(2) + " EUR");
      }
      wordTemplate.leerzeile(doc);

      wordTemplate.absatz(
          doc, "Es handelt sich um den Verzicht auf Erstattung von Aufwendungen: Nein");
      wordTemplate.leerzeile(doc);
