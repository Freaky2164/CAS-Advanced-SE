...
/**
 * @author Nils
 *     <p>Verteiler, Serienbrief-Adressen und Serienbrief-Generierung, portiert aus
 *     CReportVerteiler, CReportSerienbrief, CReportSerienbriefAdressen sowie
 *     CCommandBriefFrauenhaus/CCommandBriefFoerderverein (FHBrief.dot/FVBrief.dot). Anders als im
 *     Altsystem (Adressliste für Words natives Seriendruck-Feature, Briefkopf einzeln per
 *     Word-COM-Lesezeichen) erzeugt {@link #serienbrief} die fertig adressierten Anschreiben direkt
 *     als docx (ein Anschreiben je Empfänger).
 */
@Service
...
public class VerteilerService {
  ...
  /**
   * @author Nils
   *     <p>E-Mail-Adressen der Mitglieder mit den gegebenen Stichworten.
   */
  ...
  /**
   * @author Nils
   *     <p>Versendet eine Sammel-E-Mail an den Verteiler per BCC.
   */
  public VersandErgebnis versenden(
      Collection<String> stichworte, String traeger, String betreff, String text) {
    List<String> empfaenger = emails(stichworte);
    if (empfaenger.isEmpty()) {
      throw new ResponseStatusException(
          BAD_REQUEST, "Keine E-Mail-Empfänger zu den gewählten Stichworten gefunden");
    }
    if (absender == null || absender.isBlank()) {
      throw new ResponseStatusException(
          INTERNAL_SERVER_ERROR, "Mail-Absender ist nicht konfiguriert");
    }

    try {
      MimeMessage nachricht = mailSender.createMimeMessage();
      MimeMessageHelper helper =
          new MimeMessageHelper(nachricht, false, StandardCharsets.UTF_8.name());
      ...
      throw new ResponseStatusException(
          INTERNAL_SERVER_ERROR, "E-Mail konnte nicht versendet werden: " + fehlermeldung(e), e);
    }
  }

  /**
   * @author Nils
   *     <p>Serienbrief-Adressliste als xlsx (Datenquelle für den Seriendruck).
   */
  public byte[] adressen(Collection<String> stichworte) {
    Workbook wb = ExcelUtil.neuesWorkbook("Serienbrief-Adressen");
    Sheet sheet = wb.getSheetAt(0);
    int line = 0;
    ExcelUtil.headerZeile(
        sheet,
        ExcelUtil.headerStyle(wb),
        line++,
        "Anrede",
        "Vorname",
        "Name",
  ...
  /**
   * @author Nils
   *     <p>Serienbrief-Generierung: ein fertig adressiertes Anschreiben je Empfänger im Verteiler
   *     (alt: CCommandBriefFrauenhaus/CCommandBriefFoerderverein + FHBrief.dot/FVBrief.dot).
   *     Briefkopf, Anschrift und Briefanrede werden vorausgefüllt; der eigentliche Brieftext wird –
   *     wie zuvor auch – anschließend in Word ergänzt.
   */
  public byte[] serienbrief(Collection<String> stichworte, String verein) {
    ...
    for (int i = 0; i < empfaenger.size(); i++) {
      ...
      String zeile1 =
          ((m.getVorname() == null || m.getVorname().isBlank()) ? "" : m.getVorname() + " ")
              + m.getName();
      String plzOrt =
          ((m.getPlz() == null || m.getPlz().isBlank()) ? "" : m.getPlz() + " ")
      ...
      String briefanrede =
          (m.getBriefanrede() == null || m.getBriefanrede().isBlank())
