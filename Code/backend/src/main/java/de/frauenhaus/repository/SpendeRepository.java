...
/**
 * @author Nils
 *     <p>Datenzugriff für {@link Spende}, mit Abfragen für die Jahresübersicht und für die
 *     Summenbildung bei Dauerspenden-Quittungen.
 */
public interface SpendeRepository extends JpaRepository<Spende, Long> {

  /**
   * @author Nils
   *     <p>Generische Suche über die wichtigsten Textfelder der Spendenliste.
   */
  @Query(
      value =
          """
  ...
  /**
   * @author Nils
   *     <p>Spendenübersicht eines Jahres (alt: CReportSpendenUebersicht).
   */
  @Query(
      """
            SELECT s FROM Spende s
  ...
  /**
   * @author Nils
   *     <p>Alle Einzelspenden eines Mitglieds im Jahr für einen Spendentyp/Träger – Summenbildung
   *     für Dauerspenden-Quittungen (alt: fillDonationSummary).
   */
  @Query(
      """
            SELECT s FROM Spende s
