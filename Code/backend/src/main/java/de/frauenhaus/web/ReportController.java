...
/**
 * @author Nils
 *     <p>REST-Endpunkte für die Report-Generierung: Übersichten als xlsx, Bußgeldbestätigungen und
 *     Spendenbescheinigungen als Word-Dokumente aus den Vorlagen in vorlagen/, Serienbriefe und
 *     Stichwortsuche sowie der SMTP-Verteiler-Versand.
 */
@RestController
...
public class ReportController {
  ...
  public ReportController(
      BussgeldReportService bussgeldReports,
  ...
  /**
   * @author Nils
   *     <p>Liefert die Bußgeld-Übersicht (Summen je Gericht und Träger) im gegebenen Zeitraum als
   *     xlsx.
   */
  ...
  /**
   * @author Nils
   *     <p>Liefert die Bußgeld-Detailliste mit Zahlungseingängen im Zeitraum für einen Träger als
   *     xlsx.
   */
  ...
  /**
   * @author Nils
   *     <p>Liefert die Zahlungsbestätigung an das Gericht (Vorlage FHBG.dot/FVBG.dot) als
   *     Word-Dokument.
   */
  @GetMapping("/bussgeld-bestaetigung/{bussgeldId}")
  public ResponseEntity<byte[]> bussgeldBestaetigung(@PathVariable Long bussgeldId) {
    return download(
        documentCreationService.bussgeldBestaetigung(bussgeldId),
        "bestaetigung-" + bussgeldId + ".doc",
        DOC);
  }

  /**
   * @author Nils
   *     <p>Liefert alle Spenden eines Jahres, gruppiert nach Träger/Spendentyp/-art, als xlsx.
   */
  ...
  /**
   * @author Nils
   *     <p>Liefert die Spendenbescheinigung (Vorlagen FHSB*.dot/FVSB*.dot je Träger und Spendentyp)
   *     als Word-Dokument.
   */
  @GetMapping("/spendenquittung/{spendeId}")
  public ResponseEntity<byte[]> spendenquittung(@PathVariable Long spendeId) {
    return download(
        documentCreationService.spendenBescheinigung(spendeId),
        "spendenbescheinigung-" + spendeId + ".doc",
        DOC);
  }

  /**
   * @author Nils
   *     <p>Liefert die E-Mail-Adressen aller Mitglieder mit den gegebenen Verteiler-Stichworten.
   */
  ...
  /**
   * @author Nils
   *     <p>Versendet eine Sammel-E-Mail an alle Verteiler-Empfänger per BCC.
   */
  @PostMapping("/verteiler/versenden")
  public VerteilerService.VersandErgebnis verteilerVersenden(
      @Valid @RequestBody VerteilerVersandRequest request) {
    return verteilerService.versenden(
        request.stichworte(), request.traeger(), request.betreff(), request.text());
  }

  /**
   * @author Nils
   *     <p>Liefert die Serienbrief-Adressliste zu den gegebenen Verteiler-Stichworten als xlsx.
   */
  ...
  /**
   * @author Nils
   *     <p>Liefert die generierten Serienbriefe (ein Anschreiben je Empfänger) als docx.
   */
  @GetMapping("/serienbrief")
  public ResponseEntity<byte[]> serienbrief(
      @RequestParam List<String> stichworte, @RequestParam String verein) {
    return download(verteilerService.serienbrief(stichworte, verein), "serienbrief.docx", DOCX);
  }

  /**
   * @author Nils
   *     <p>Liefert die Mitglieder, die mindestens eines der gegebenen Stichworte tragen, als
   *     Vorschau (alt: CReportStichwortSuche).
   */
  ...
  /**
   * @author Nils
   *     <p>Liefert die Mitgliedersuche über Stichworte als xlsx-Download (alt:
   *     CReportStichwortSuche).
   */
  ...
  public ResponseEntity<byte[]> stichwortsucheExcel(
      ...
      @RequestParam(defaultValue = "false") boolean frauenhaus) {
    return download(
        stichwortsucheService.suchenAlsExcel(stichworte, foerderverein, frauenhaus),
        "stichwortsuche.xlsx",
        XLSX);
  }
  ...
  /**
   * @author Nils
   *     <p>Baut die Download-Antwort mit Dateinamen und Content-Type für den gegebenen
   *     Report-Inhalt.
   */
