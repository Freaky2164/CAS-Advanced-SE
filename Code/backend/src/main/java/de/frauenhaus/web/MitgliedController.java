...
/**
 * @author Nils
 *     <p>REST-Endpunkte zur Pflege der Mitglieder/Adressen (Stammdaten-CRUD), inkl. Duplizieren
 *     (alt: CInfoFrameStatusCopy) und Verteiler-/Vereinszuordnung.
 */
@RestController
...
public class MitgliedController {
  ...
  /**
   * @author Nils
   *     <p>Anfrage zum Anlegen/Ändern eines Mitglieds; {@code stichworte}/{@code vereine} sind
   *     optional.
   */
  ...
  @GetMapping
  public Page<MitgliedResponse> alle(
  ...
  public ResponseEntity<MitgliedResponse> anlegen(@Valid @RequestBody MitgliedRequest request) {
    MitgliedResponse angelegt =
        mitgliedService.anlegen(request.toEntity(), request.stichworte(), request.vereine());
    return ResponseEntity.status(HttpStatus.CREATED).body(angelegt);
  }

  @PutMapping("/{id}")
  public MitgliedResponse aendern(
      @PathVariable Long id, @Valid @RequestBody MitgliedRequest request) {
    return mitgliedService.aendern(id, request.toEntity(), request.stichworte(), request.vereine());
  }

  /**
   * @author Nils
   *     <p>Dupliziert ein Mitglied inkl. Stammdaten und Zuordnungen.
   */
