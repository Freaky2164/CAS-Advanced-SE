...
/**
 * @author Nils
 *     <p>Integrationstests für die generische Stammdaten-Suche über die Listen-Endpunkte.
 */
@SpringBootTest
@Transactional
...
class StammdatenSucheIntegrationTest {
  ...
  @Autowired private MitgliedService mitgliedService;
  @Autowired private SpendeService spendeService;
  @Autowired private BussgeldService bussgeldService;
  @Autowired private VereinService vereinService;
  @Autowired private GerichtService gerichtService;

  @Autowired private MitgliedRepository mitglieder;
  @Autowired private StichwortRepository stichworte;
  @Autowired private SpendeRepository spenden;
  @Autowired private SpendenartRepository spendenarten;
  @Autowired private BussgeldRepository bussgelder;
  @Autowired private SpendentypRepository spendentypen;
  @Autowired private BussgeldstatusRepository bussgeldstati;
  @Autowired private VereinRepository vereine;
  @Autowired private GerichtRepository gerichte;
  ...
  void mitgliederSucheFiltertUeberStichwortUndIstCaseInsensitive() {
    ...
    Mitglied treffer =
        neuesMitglied(
            "Anna", "MitgliedSuche" + suffix, "Zuerich", "mitglied-" + suffix + "@test.invalid");
    treffer.getStichworte().add(stichworte.save(new Stichwort("AlphaSuche" + suffix)));
    mitglieder.saveAndFlush(treffer);

    Mitglied ohneTreffer =
        neuesMitglied("Berta", "OhneTreffer" + suffix, "Bern", "ohne-" + suffix + "@test.invalid");
    mitglieder.saveAndFlush(ohneTreffer);

    var seite = mitgliedService.alle(PageRequest.of(0, 20), ("alphasuche" + suffix).toLowerCase());
    assertThat(seite.getContent())
        .extracting(MitgliedService.MitgliedResponse::id)
        .containsExactly(treffer.getId());

    long erwartet = mitglieder.count();
    assertThat(mitgliedService.alle(PageRequest.of(0, 20), null).getTotalElements())
        .isEqualTo(erwartet);
    assertThat(mitgliedService.alle(PageRequest.of(0, 20), "   ").getTotalElements())
        .isEqualTo(erwartet);
  }
  ...
  void spendenSucheFiltertNachMitgliedUndIstCaseInsensitive() {
    ...
    Spendenart spendenart =
        spendenarten.saveAndFlush(new Spendenart("Geldspende " + suffix, spendentyp.getName()));
    Mitglied trefferMitglied =
        mitglieder.saveAndFlush(neuesMitglied("Clara", "SpendeSuche" + suffix, "Basel", null));
    Mitglied ohneTrefferMitglied =
        mitglieder.saveAndFlush(neuesMitglied("Dora", "AndereSpende" + suffix, "Luzern", null));

    spenden.saveAndFlush(neueSpende(trefferMitglied, spendenart, verein, "Bemerkung " + suffix));
    spenden.saveAndFlush(
        neueSpende(ohneTrefferMitglied, spendenart, verein, "Andere Bemerkung " + suffix));

    var seite = spendeService.alle(PageRequest.of(0, 20), ("spendesuche" + suffix).toLowerCase());
    assertThat(seite.getContent())
        .extracting(SpendeService.SpendeResponse::mitgliedName)
        .allMatch(name -> name.contains("SpendeSuche" + suffix))
        .hasSize(1);

    long erwartet = spenden.count();
    assertThat(spendeService.alle(PageRequest.of(0, 20), null).getTotalElements())
        .isEqualTo(erwartet);
    assertThat(spendeService.alle(PageRequest.of(0, 20), "").getTotalElements())
        .isEqualTo(erwartet);
  }
  ...
  void bussgeldSucheFiltertNachAktenzeichenUndIstCaseInsensitive() {
    ...
    assertThat(seite.getContent())
        .extracting(BussgeldService.BussgeldResponse::aktenzeichen)
        .containsExactly(treffer.getAktenzeichen());

    long erwartet = bussgelder.count();
    assertThat(bussgeldService.alle(PageRequest.of(0, 20), null).getTotalElements())
        .isEqualTo(erwartet);
    assertThat(bussgeldService.alle(PageRequest.of(0, 20), " ").getTotalElements())
        .isEqualTo(erwartet);
  }
  ...
  void vereineSucheFiltertNachBezeichnungUndIstCaseInsensitive() {
    String suffix = suffix();
    Verein treffer =
        vereine.saveAndFlush(new Verein("VV" + suffix, "Foerderverein Suche " + suffix));
  ...
  void gerichteSucheFiltertNachOrtUndIstCaseInsensitive() {
    String suffix = suffix();
    Gericht treffer =
        gerichte.saveAndFlush(neuesGericht("Landgericht " + suffix, "HamburgSuche" + suffix));
  ...
  private Spende neueSpende(
      Mitglied mitglied, Spendenart spendenart, Verein verein, String bemerkung) {
  ...
  private Bussgeld neuesBussgeld(
      Gericht gericht,
      Verein verein,
      String status,
      String aktenzeichen,
