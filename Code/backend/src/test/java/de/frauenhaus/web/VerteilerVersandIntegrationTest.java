...
/**
 * @author Nils
 *     <p>Integrationstests für den SMTP-basierten Verteiler-Versand.
 */
@SpringBootTest
...
class VerteilerVersandIntegrationTest {
  ...
  void versendetEineSammelmailMitBccUndBetreff() throws Exception {
    ...
    mitglieder.saveAndFlush(
        mitgliedMitStichwort(
            "Anna", "Alpha" + suffix, "anna-" + suffix + "@test.invalid", stichwort));
    mitglieder.saveAndFlush(
        mitgliedMitStichwort(
            "Berta", "Beta" + suffix, "berta-" + suffix + "@test.invalid", stichwort));
    mitglieder.saveAndFlush(mitgliedMitStichwort("Clara", "OhneMail" + suffix, null, stichwort));

    mockMvc
        .perform(
            post("/api/reports/verteiler/versenden")
    ...
    assertThat(nachricht.getContent().toString())
        .contains("Liebe Unterstuetzerinnen und Unterstuetzer");
    assertThat(adressen(nachricht.getRecipients(Message.RecipientType.TO)))
        .containsExactly("no-reply@localhost");
  ...
  void meldetKeineEmpfaengerAlsBadRequest() throws Exception {
    mockMvc
        ...
        .andExpect(
            jsonPath("$.detail")
                .value("Keine E-Mail-Empfänger zu den gewählten Stichworten gefunden"));
  }
  ...
  void uebersetztMailFehlerInSaubereApiAntwort() throws Exception {
    ...
    mitglieder.saveAndFlush(
        mitgliedMitStichwort(
            "Dora", "Delta" + suffix, "dora-" + suffix + "@test.invalid", stichwort));
  ...
  private Mitglied mitgliedMitStichwort(
      String vorname, String name, String email, Stichwort stichwort) {
