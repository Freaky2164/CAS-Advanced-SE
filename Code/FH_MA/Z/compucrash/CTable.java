...
/*		hdr.addMouseListener(new MouseAdapter() {
	...
	public CProperties getKeys() {
		CTableModel tabModel = (CTableModel)getModel();
		CProperties keys = tabModel.getKeys(getSelectedRow());
		if (keys == null) {
			keys = new CProperties();
			CProperties keyA = new CProperties();
			try {
// TODO Hier evtl auf Properties abfragen. Besser auch wegen der Spaltenbreiten.
				keyA.put("owner", tabModel.getMetaData().getSchemaName(0));
				keyA.put("table_name", tabModel.getMetaData().getTableName(0));
				keyA.put("column_name", tabModel.getMetaData().getColumnName(0));
//				keyA.put("value", tabModel.getValueAt(getSelectedRow(),0).toString());
				keyA.put("value", tabModel.getValueAt(getSelectedRow(),0));
	...
	public void getAsCSV() {
		...
		try {
			...
			for (int i = 0; i < getRowCount(); i++) {
				output = new String();
/*				
 * DIeser CodeTeil ist erst ab 1.5.0 möglich
				for (int j = 0; j < getColumnCount(); j++) {
...
// Bis hierher Ersatz für 1.4.1_07
