...
public class CInfoFrameStatusSub extends CInfoFrameStatus {
	...
	public void entry() {
		...
		CProperties ps = (CProperties)owner.p.get("sub");
		for (int i = 0; i < owner.cFields.size(); i++) {
			...
			for (int j = 1; j <= ps.size(); j++) {
				...
				if (psa.get("column_name").toString().equalsIgnoreCase(pA.get("column_name").toString())
					...
					((CDisplayField) owner.cFields.get(i)).setValue(psa.get("value"));					
				}
			}
			if (pA.get("init") != null) {
				((CDisplayField) owner.cFields.get(i)).setValue(CDataManager.getInstance().getInit((String)pA.get("init")));
			}
		}
	}
			...
			if (!pA.get("iskey").toString().equalsIgnoreCase("0")) {
				...
				for (int k = 0; k < owner.cFields.size(); k++) {
					...
					if (pValue.get("column_name").toString().equalsIgnoreCase(pA.get("column_name").toString()) 
					    ...
					    if (pValue.get("data_type").toString().equalsIgnoreCase("DATE")) {
					        DateTime rawDate = (DateTime)((CDisplayField) owner.cFields.get(k)).getValue();
							DateTimeFormatter fmt = DateTimeFormat.forPattern("dd.MM.yyyy");
							String dateString = fmt.print(rawDate);    
					        pKey.put("value",dateString);					        
					    } else {
